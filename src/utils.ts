...
import { MintingAssetName, ValidTestnetChainInput, TestnetChainName, ValidMainnetChainInput, MainnetChainName } from "./types";
...
export function getMainnetAssetAddress(assetName: MintingAssetName, chain: ValidMainnetChainInput): string {
    // Convert chain ID to chain name if needed
    let chainName: MainnetChainName;
  
    if (typeof chain === "number") {
      // Find chain name by chain ID
      const foundChainName = Object.entries(CHAIN_NAME_CHAIN_ID_MAP).find(
        ([, chainId]) => chainId === chain
      )?.[0] as MainnetChainName;
